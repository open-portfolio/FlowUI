...
public let userAgreedTermsAtKey = "userAgreedTermsAt"

public struct TermsView: View {
    private static let formatter = ISO8601DateFormatter()
    @AppStorage(userAgreedTermsAtKey) var agreedTerms: String = ""

    @Binding var showTerms: Bool
    var onUpdate: () -> Void

    public init(showTerms: Binding<Bool>, onUpdate: @escaping () -> Void) {
        _showTerms = showTerms
        self.onUpdate = onUpdate
    }

    let llc = "OpenAlloc LLC"

    var termsText: LocalizedStringKey {
        """
        ‘The software’ means the _\(appName)_ application.
        ...
        The software is for educational purposes only. It does not provide investment or tax advice. Its developers (_\(llc)_ and project contributors) are not advisers and do not offer investment or tax advice. Before making any investment decisions from use of the software, you should carefully inspect the output to ensure the calculations are fully consistent with your investment objectives.
    ...
    let licenseText: LocalizedStringKey =
        """
    ...
    public var body: some View {
        VStack {
            ...
            ScrollView {
                Text(termsText)
                    .lineLimit(nil)
                    .textSelection(.enabled) // macOS 12.0+
            ...
            // TODO: why isn't this working in toolbar
            #if os(iOS)
                agreeButton
            #endif
        }
        .frame(minWidth: 400, idealWidth: 600, maxWidth: .infinity, maxHeight: .infinity) // , alignment: .topLeading)
        .padding()
//        .modify {
//            #if os(iOS)
//            $0.navigationBarItems(trailing: acknowledgeButton)
//            #endif
//        }
        .toolbar {
            #if os(macOS)
...
//                closeButton
    ...
    private var agreeButton: some View {
        Button(action: {
            agreedTerms = TermsView.formatter.string(from: Date())
            showTerms = false
            onUpdate()
        }) {
            Text("Agree")
    ...
    private var copyToClipboardButton: some View {
        Button(action: {
            #if os(macOS)
                let fullText = "\(appName) Terms and Conditions\n\n\(termsText)\n\n\(licenseText)"
    ...
    private var appName: String {
        Bundle.main.applicationName ?? "[This Application]"
    }
}
